...
| ANALYTICS-001 | S1 | Analytics/LGPD | Reduzido localmente; validacao remota pendente | Etapa 7 prepara persistencia somente com `product_analytics_v1`, allowlist, metadata minimizada e retencao 90 dias; PR de hardening moveu insert para action server-side/admin e migration aditiva revoga insert direto de anon/autenticado; falta aplicacao/typegen e preview fresco | Validar tabelas/policies em Supabase preview aprovado e rodar harness/smoke | Preview confirma ausencia/revogacao de consentimento bloqueia evento, insert direto por cliente falha e user B/Atalaia/anon nao acessam |
| AI-CONSENT-AUDIT-001 | S1 | IA/LGPD | Reduzido localmente; validacao real pendente | `invokeAiWithPersistentConsentAndAudit` consulta `user_preferences`/`consent_records`, usa versoes `ai_provider_openai_v1`/`ai_provider_deepseek_v1`, bloqueia provider sem sessao/auditoria viavel e persiste `ai_run_audits` via admin server-only; migration local alinha constraints ao `ai_run_audit_v1` | Aplicar migration em preview aprovado, validar RLS/Auth, rodar eval/smoke de provider real autorizado e contador diario persistente | IA real so chama provider quando consentimento versionado/revogavel e auditoria minima persistida estiverem validados em ambiente aprovado |
...
| ID | Status | Evidencia |
|---|---|---|
| AUTH-SSR-001 | Reduzido localmente, ainda S1 externo | Proxy de request fica em `src/proxy.ts`, `/onboarding` e protegido fora de `local-demo`, e `/api/health`/`/api/ready` continuam alcancando os handlers publicos. Fechamento exige URL HTTPS/Auth real. |
| AUTH-ONBOARDING-001 | Reduzido localmente | `saveOnboarding` falha fechado fora de `local-demo` quando sessao/configuracao real falta, sem mascarar como draft salvo. |
| ANALYTICS-001 | Reduzido localmente, ainda S1 remoto | Persistencia usa admin server-only apos consentimento/sanitizacao; migration `20260605130314_analytics_feedback_server_only_persistence.sql` revoga insert direto de anon/autenticado. Fechamento exige preview/typegen/harness fresco. |
| FEEDBACK-REAL-001 | Reduzido localmente, ainda S2 remoto | Persistencia usa admin server-only apos aviso/consentimento/bloqueio sensivel; insert direto de anon/autenticado fica revogado. Fechamento exige preview/harness fresco e politica de canal externo se houver. |

## PR - consentimento e auditoria persistida de IA

Data: 2026-06-05.

| ID | Status | Evidencia |
|---|---|---|
| AI-CONSENT-AUDIT-001 | Reduzido localmente, ainda S1 externo | Roteamento passa a usar consentimentos versionados por provider (`ai_provider_openai_v1`, `ai_provider_deepseek_v1`); `invokeAiWithPersistentConsentAndAudit` carrega preferencia/consentimentos persistidos, bloqueia chamada real sem sessao/auditoria viavel e grava `ai_run_audits` via admin server-only. Fechamento exige preview/Auth/RLS e provider real autorizado. |
