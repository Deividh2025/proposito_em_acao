...
export async function generateMetacognitionReflection(
  input: unknown
...
export async function persistMetacognitionSession(
  input: unknown
): Promise<MetacognitionActionResult> {
  ...
  try {
    ...
    const ownershipChecks = [
      parsed.input.relatedTaskId
        ? belongsToUser(supabase, "tasks", parsed.input.relatedTaskId, user.id)
        : true,
      parsed.input.relatedProjectId
        ? belongsToUser(supabase, "projects", parsed.input.relatedProjectId, user.id)
        : true,
      parsed.input.relatedGoalId
        ? belongsToUser(supabase, "goals", parsed.input.relatedGoalId, user.id)
        : true
    ];
...
export async function listMetacognitionHistory(input: unknown = {}) {
  ...
  return listMetacognitionHistoryForCurrentUser(parsed.limit);
}
