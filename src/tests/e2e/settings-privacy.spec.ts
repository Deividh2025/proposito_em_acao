...
test("Etapa 7 settings renders privacy controls and saves local-demo preferences", async ({
  ...
  await page.getByRole("radio", { name: /Preferencia explicita/ }).check();
  ...
  await expect(page.getByText("IA selecionada").locator("..")).toContainText("deepseek");
...
test("Etapa 7 settings gates analytics and deletion with explicit consent", async ({
  ...
  const deepseekConsent = page
    .getByText("Entendo e autorizo somente ai_provider_deepseek_v1.")
    .locator("xpath=ancestor::section[1]");

  await deepseekConsent.getByLabel("Entendo e autorizo somente ai_provider_deepseek_v1.").check();
  await deepseekConsent.getByRole("button", { name: "Autorizar" }).click();
